...
for entry in "${DISKS[@]}"; do
    ...
    # Пропускаем устройства меньше 32 ГБ (вероятные флешки)
    if [ "$SIZE" -lt $((32*1024*1024*1024)) ]; then
        echo "Пропущено устройство: $DEV (слишком маленький размер: $((SIZE / 1024 / 1024 / 1024)) ГБ)"
        continue
    fi
